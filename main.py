...
async def start(message: Message):
    ...
    if user_id == ADMIN:  # Admin uchun maxsus tugmalar
        ...
        await message.answer(
            "Assalomu aleykum, botga xush kelibsiz!", reply_markup=inline_btn
...
async def inline_button_handler(callback_query: types.CallbackQuery):
    await callback_query.message.answer_photo("https://unsplash.com/photos/empty-building-pathway-lDxxeAJrWpE", caption="Assalomu Aleykum botga xush kelibsiz sizni botda ko‘rishdan hursandmiz☺️\n\nSiz bu botda EFendida joylarni band qilishingiz va oldindan taom buyurtma qilishingiz mumkin🍔\n\n/Xonalar 👈 bu buyruq sizga 15 ta xonalar ro‘yxatini taqdim etadi\n\n\nBatafsil ma’lumot olish uchun @Rahimov_MR tg profilga yozing yoki tel:+998993666674 telefon qiling")
    await bot.answer_callback_query(callback_query.id)



@router.callback_query(lambda c: c.data == "bo'sh")
async def inline_button_handler(callback_query: types.CallbackQuery):
    # Band qilingan joylarni olish
    cursor.execute("SELECT xona, time, username FROM user_times")
    results = cursor.fetchall()

    if not results:
        await callback_query.message.answer("Hozirda band qilingan joylar yo'q.")
        return

    # Javobni formatlash
    response = "Band qilingan joylar ro‘yxati:\n\n"
    for row in results:
        xona, time, username = row
        response += f"🏠 Xona: {xona}\n🕒 Vaqt: {time}\n👤 Foydalanuvchi: @{username if username else 'Noma’lum'}\n\n"
...
@dp.message(F.text == "❎Bant qilingan joylarni korish")
async def inline_button_handler(message: Message):
    if message.from_user.id == ADMIN:
        ...
        if not results:
            await message.answer("Hozirda band qilingan joylar yo'q.")
            return
...
async def joy_1_handler(message: Message):
    ...
    async def yes_handler(callback_query: types.CallbackQuery):
        ...
        await callback_query.message.answer(
            "1-xona tanlandi. Soat nechaga band qilasiz?\n\nMisol: 15:25"
        )
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            for row in result:
                existing_time = row[0]
                if existing_time == time_input:
                    conflicting_time = existing_time
                    break

            if conflicting_time:
                await message.answer(f"Kechirasiz, 1-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
...
async def joy_2_handler(message: Message):
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            if conflicting_time:
                await message.answer(f"Kechirasiz, 2-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
            ...
            try:
                ...
                cursor.execute(
                    "INSERT INTO user_times (user_id, username, xona, time) VALUES (?, ?, ?, ?)",
                    (message.from_user.id, message.from_user.username, 2, time_input)
                )
                conn.commit()
                await message.answer(f"Vaqt '{time_input}' muvaffaqiyatli saqlandi!")
                await message.answer(
                    f"Sizning chekingiz:🧾\n\nFoydalanuvchi ismi👤: {message.from_user.full_name}\n\n🏠Band qilingan joy: 2-Xona\n\n🕒Vaqt: {time_input}")
                # Botdan foydalanuvchiga xabar yuborish
                admin_message = (
                    f"Salom ADMIN sizga xabar bor‼️\n\nFoydalanuvchi ismi👤: {message.from_user.full_name}\n\nFoydalanuvchi nomi: @{message.from_user.username}\n\n🏠Band qilingan joy: 2-Xona\n\n🕒Vaqt: {time_input}")
                ...
                # Bu yerda xabar yuborish codi tugatildi
            except sqlite3.Error as e:
                await message.answer(f"Xatolik yuz berdi: {e}")
        else:
            await message.answer("Xatolik! Iltimos, vaqtni to‘g‘ri formatda kiriting (masalan, 15:23).")
...
async def joy_3_handler(message: Message):
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            if conflicting_time:
                await message.answer(f"Kechirasiz, 3-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
            ...
            try:
                ...
                cursor.execute(
                    "INSERT INTO user_times (user_id, username, xona, time) VALUES (?, ?, ?, ?)",
                    (message.from_user.id, message.from_user.username, 3, time_input)
                )
                conn.commit()
                await message.answer(f"Vaqt '{time_input}' muvaffaqiyatli saqlandi!")
                await message.answer(
                    f"Sizning chekingiz:🧾\n\nFoydalanuvchi ismi👤: {message.from_user.full_name}\n\n🏠Band qilingan joy: 3-Xona\n\n🕒Vaqt: {time_input}")
                # Botdan foydalanuvchiga xabar yuborish
                admin_message = (
                    f"Salom ADMIN sizga xabar bor‼️\n\nFoydalanuvchi ismi👤: {message.from_user.full_name}\n\nFoydalanuvchi nomi: @{message.from_user.username}\n\n🏠Band qilingan joy: 3-Xona\n\n🕒Vaqt: {time_input}")
...
async def joy_3_handler(message: Message):
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            if conflicting_time:
                await message.answer(f"Kechirasiz, 4-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
            ...
            try:
                ...
                cursor.execute(
                    "INSERT INTO user_times (user_id, username, xona, time) VALUES (?, ?, ?, ?)",
                    (message.from_user.id, message.from_user.username, 4, time_input)
                )
                conn.commit()
                await message.answer(f"Vaqt '{time_input}' muvaffaqiyatli saqlandi!")
                await message.answer(
                    f"🧾Sizning chekingiz:\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\n🏠Band qilingan joy: 4-Xona\n\n🕒Vaqt: {time_input}")
                # Botdan foydalanuvchiga xabar yuborish
                admin_message = (
                    f"Salom ADMIN sizga xabar bor‼️\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\nFoydalanuvchi nomi: @{message.from_user.username}\n\n🏠Band qilingan joy: 4-Xona\n\n🕒Vaqt: {time_input}")
...
async def joy_3_handler(message: Message):
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            if conflicting_time:
                await message.answer(f"Kechirasiz, 5-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
            ...
            try:
                ...
                cursor.execute(
                    "INSERT INTO user_times (user_id, username, xona, time) VALUES (?, ?, ?, ?)",
                    (message.from_user.id, message.from_user.username, 6, time_input)
                )
                conn.commit()
                await message.answer(f"Vaqt '{time_input}' muvaffaqiyatli saqlandi!")
                await message.answer(
                    f"🧾Sizning chekingiz:\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\n🏠Band qilingan joy: 5-Xona\n\n🕒Vaqt: {time_input}")
                # Botdan foydalanuvchiga xabar yuborish
                admin_message = (
                    f"Salom ADMIN sizga xabar bor‼️\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\nFoydalanuvchi nomi: @{message.from_user.username}\n\n🏠Band qilingan joy: 5-Xona\n\n🕒Vaqt: {time_input}")
...
async def joy_3_handler(message: Message):
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            if conflicting_time:
                await message.answer(f"Kechirasiz, 6-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
...
async def joy_3_handler(message: Message):
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            if conflicting_time:
                await message.answer(f"Kechirasiz, 7-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
            ...
            try:
                ...
                cursor.execute(
                    "INSERT INTO user_times (user_id, username, xona, time) VALUES (?, ?, ?, ?)",
                    (message.from_user.id, message.from_user.username, 7, time_input)
                )
                conn.commit()
                await message.answer(f"Vaqt '{time_input}' muvaffaqiyatli saqlandi!")
                await message.answer(
                    f"🧾Sizning chekingiz:\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\n🏠Band qilingan joy: 7-Xona\n\n🕒Vaqt: {time_input}")
                # Botdan foydalanuvchiga xabar yuborish
                admin_message = (
                    f"Salom ADMIN sizga xabar bor‼️\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\nFoydalanuvchi nomi: @{message.from_user.username}\n\n🏠Band qilingan joy: 7-Xona\n\n🕒Vaqt: {time_input}")
...
async def joy_3_handler(message: Message):
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            if conflicting_time:
                await message.answer(f"Kechirasiz, 8-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
            ...
            try:
                ...
                cursor.execute(
                    "INSERT INTO user_times (user_id, username, xona, time) VALUES (?, ?, ?, ?)",
                    (message.from_user.id, message.from_user.username, 8, time_input)
                )
                conn.commit()
                await message.answer(f"Vaqt '{time_input}' muvaffaqiyatli saqlandi!")
                await message.answer(
                    f"🧾Sizning chekingiz:\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\n🏠Band qilingan joy: 8-Xona\n\n🕒Vaqt: {time_input}")
                # Botdan foydalanuvchiga xabar yuborish
                admin_message = (
                    f"Salom ADMIN sizga xabar bor‼️\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\nFoydalanuvchi nomi: @{message.from_user.username}\n\n🏠Band qilingan joy: 8-Xona\n\n🕒Vaqt: {time_input}")
...
async def joy_3_handler(message: Message):
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            if conflicting_time:
                await message.answer(f"Kechirasiz, 9-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
            ...
            try:
                ...
                cursor.execute(
                    "INSERT INTO user_times (user_id, username, xona, time) VALUES (?, ?, ?, ?)",
                    (message.from_user.id, message.from_user.username, 9, time_input)
                )
                conn.commit()
                await message.answer(f"Vaqt '{time_input}' muvaffaqiyatli saqlandi!")
                await message.answer(
                    f"🧾Sizning chekingiz:\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\n🏠Band qilingan joy: 9-Xona\n\n🕒Vaqt: {time_input}")
                # Botdan foydalanuvchiga xabar yuborish
                admin_message = (
                    f"Salom ADMIN sizga xabar bor‼️\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\nFoydalanuvchi nomi: @{message.from_user.username}\n\n🏠Band qilingan joy: 9-Xona\n\n🕒Vaqt: {time_input}")
...
async def joy_3_handler(message: Message):
    ...
    async def handle_time_input(message: Message):
        ...
        if validate_time_format(time_input):
            ...
            if conflicting_time:
                await message.answer(f"Kechirasiz, 10-xona ushbu vaqtda band qilingan: {conflicting_time}")
                return
            ...
            try:
                ...
                cursor.execute(
                    "INSERT INTO user_times (user_id, username, xona, time) VALUES (?, ?, ?, ?)",
                    (message.from_user.id, message.from_user.username, 10, time_input)
                )
                conn.commit()
                await message.answer(f"Vaqt '{time_input}' muvaffaqiyatli saqlandi!")
                await message.answer(
                    f"🧾Sizning chekingiz:\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\n🏠Band qilingan joy: 10-Xona\n\n🕒Vaqt: {time_input}")
                # Botdan foydalanuvchiga xabar yuborish
                admin_message = (
                    f"Salom ADMIN sizga xabar bor‼️\n\n👤Foydalanuvchi ismi: {message.from_user.full_name}\n\nFoydalanuvchi nomi: @{message.from_user.username}\n\n🏠Band qilingan joy: 10-Xona\n\n🕒Vaqt: {time_input}")
...
@dp.message(F.text.startswith("✂️O'chirish"))
